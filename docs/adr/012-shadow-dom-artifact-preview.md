...
Accepted。Shadow DOM 固定逻辑视口与桌面 `80%` 视觉缩放均已实施。
...
6. 桌面工作台保持固定三栏最小宽度，空间不足时由外层产生横向滚动，不压缩手机逻辑内容。
7. 手机逻辑视口保持 `390 × 844px`，完整设备外框在桌面工作台中固定以 `80%` 视觉比例展示；右侧栏固定为 `400px`。
...
- 完整设备外层固定缩放可以减少辅助预览对主创作区的占用，同时不引入第二套排版结果。
...
- CSS transform 不改变布局占位：使用显式 `DeviceStage` 承载缩放后的尺寸，避免右栏残留空白或出现额外溢出。
- 浏览器无法保证实体毫米尺寸一致：以稳定的长宽比例、`80%` 桌面视觉比例和固定逻辑视口作为可验证目标。
