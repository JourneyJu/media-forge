...
| 固定预览 | 右侧固定 `400px`；手机逻辑内容固定 `390 × 844px`，完整设备以 `80%` 显示，桌面分辨率变化不改变文章换行和图片位置。 |
...
- 不允许通过 `dvh`、百分比、容器压缩或 `zoom` 缩小手机文章逻辑布局宽度；`scale(0.8)` 只能作用于完成排版后的完整设备外层。
...
- 截图和 DOM 断言共同验证：手机逻辑内容宽度始终为 `390px`，完整设备视觉尺寸约为 `328 × 691.2px`，中间区不小于 `720px`，右侧为 `400px`；同一 Artifact 的标题换行、段落行数和图片位置一致。
- 在 `1920 × 1080` 下检查兼容性提示完整可见且右栏无过量留白；在 `1440px` 下检查展开状态三栏基本完整显示。
